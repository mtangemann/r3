...
# the key scheme) that reconciliation classifies enumerated keys against — deliberately
# above the thin transport primitives.
...
class RemoteCheckReport:
    ...
    resurrection_risks: List[RemoteCheckFinding] = field(default_factory=list)
    #: Job prefixes with archive/sidecar objects but no ``manifest.json``.
    manifestless_prefixes: List[RemoteCheckFinding] = field(default_factory=list)
    #: Leftover ``manifest.json.staging`` objects (interrupted publishes).
    staging_manifests: List[RemoteCheckFinding] = field(default_factory=list)
    #: Index rows on a remote whose manifest/archive is missing or inconsistent.
    broken_rows: List[RemoteCheckFinding] = field(default_factory=list)
    #: Incomplete multipart uploads wasting quota under a remote's prefix.
    incomplete_multipart_uploads: List[MultipartUploadFinding] = field(
...
class Repository:
    ...
    def remote_check(self) -> RemoteCheckReport:
        ...
        """
        report = RemoteCheckReport()
        for remote_name, remote in self._remotes.items():
            self._check_remote(remote_name, remote, report)
        return report

    def _check_remote(
        self, remote_name: str, remote: Remote, report: RemoteCheckReport
    ) -> None:
        ...
        for job_id, names in objects.items():
            if _MANIFEST_NAME in names:
                ...
                self._classify_complete_manifest(remote_name, job_id, report)
            elif names & _PAYLOAD_NAMES and not self._indexed_on(job_id, remote_name):
        ...
        for job in self.find({}, location=remote_name):
            assert job.id is not None
            self._probe_remote_row(remote_name, remote, job.id, report)
        ...
        for key, upload_id in remote.list_incomplete_multipart_uploads():
            report.incomplete_multipart_uploads.append(
                MultipartUploadFinding(remote_name, key, upload_id)
            )

    def _indexed_on(self, job_id: str, remote_name: str) -> bool:
        """True iff the index has a row placing ``job_id`` on ``remote_name``."""
        try:
            return self._index.get_location(job_id) == remote_name
        except KeyError:
            return False

    def _classify_complete_manifest(
        self, remote_name: str, job_id: str, report: RemoteCheckReport
    ) -> None:
